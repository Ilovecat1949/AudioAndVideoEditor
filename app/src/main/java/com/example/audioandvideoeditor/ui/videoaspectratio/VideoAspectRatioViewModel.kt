...
//        private set
    ...
    var selectedBackgroundColor = mutableStateOf(Color.Companion.Black)
        private set
    var dimensionsReady = mutableStateOf(false)
        private set
    val info= MediaInfo()
    fun initSource(path:String) {
//        originalVideoWidth.value = width
//        originalVideoHeight.value = height
        viewModelScope.launch {
            val text = AppApplication.Companion.INSTANCE.taskRepository.getAVInfo(path)
            info.initInfo(text)
...
//    }
    ...
    fun getNewVideoDimensions(): Pair<Int, Int> {
        ...
        val h_max = Math.min(videoScreenHeight * 1f, videoScreenWidth / ratio)
